...
namespace Aufgabe11 {
    ...
    function Konfigurator(): void {                                        //Erstellt Funktion Konfigurator ohne Rückgabewert
        ...
        for (let i: number = 0; i < baumdaten.length; i++) {               //for schleife mit deklaration
            ...
            baumarten.id = baumdaten[i].element;                              //ID für späteren Nutzen im Warenkorb (Brauch man das)
        ...
        for (let i: number = 0; i < kugeldaten.length; i++) {                 //for schleife mit deklaration
            ...
            let kugelanz: HTMLInputElement = document.createElement("input");   //Variable namens kugellabel soll ein HTMLLabelelement erstellen
            ...
            kugelanz.addEventListener("change", function(): void {              //Erstelle EventListener für Variable kugelanz, reagier bei Veränderung und 
    ...
    function CheckBoxKugelnAuslesen(CheckElement: HTMLInputElement, anzahl: string): void { //Funktion mit Parametern
        for (let i: number = 0; i < kugeldaten.length; i++) {                               //For schleife mit Deklaration
            if (kugeldaten[i].element == CheckElement.id) {                                 //If abfrage ob die genannten Daten gleich sind
    ...
    function CheckBoxKerzenAuslesen(CheckElement: HTMLInputElement, anzahl: string): void { //Funktion mit Parametern
        for (let i: number = 0; i < kerzendaten.length; i++) {                              //For schleife mit Deklaration
            if (kerzendaten[i].element == CheckElement.id) {                                //If abfrage ob die genannten Daten gleich sind
    ...
    function AuswahlAuslesen(): void {                      //funktionsaufruf
        var baumname: string = baumarten.value;             //variable vom typ string ist gleich baumarten.value
        if (baumname != "") {                               //if abfrage 
            ZuWarenkorb(baumdaten, true, baumname);         //wenn baumname nicht leer ist soll baumname und baumdaten an warenkorb ausgegeben werden
        }
        else {                                              //wenn baumname leer ist wird nichts übergeben
        ...
        if (halterungname != "") {                                  //gleiche abfrage wie beim baumname
        ...
        if (lieferant != "") {                                      //gleiche abfrage wie bei halterung
            ZuWarenkorb(lieferoptionen, true, lieferant);           //warum gibt es kein else???
        }
    }
    ...
    function Warenkorb(elementId: string, value: string, preis: number, anzahl: number, selected: boolean): void { //funktion warenkorb mit parametern von drüber
        ...
        var preisElement: number;                                                       //variable preiselement ist eine nummer
        ...
        if (selected) {                                                                 //if abfrage für checkbox
            var p: HTMLParagraphElement = document.createElement("p");                  //variabel p ist ein html paragraph
        ...
        for (let i: number = 0; i < korb.getElementsByTagName("p").length; i++) {       //for schleife für die länge korb elemente
           
            if (korb.getElementsByTagName("p")[i].id == elementId) {                                //if abfrage stimmen daten überein
                var innerPreis: string = korb.getElementsByTagName("p")[i].innerText.split("=")[1]; //variable innerpreis zieht von p den text an stelle 1
    ...
    function PrufeDaten(): void {                   //funktion prüfe daten
        ...
        else {                                          //wenn die felder gefüllt sind
